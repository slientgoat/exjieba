...
priv/mp_segment.so:
	mkdir -p priv && \
	$(CC) $(CFLAGS) $(ERLANG_FLAGS) $(CPPJIEBA_FLAGS) -shared -DLOGGER_LEVEL=LL_ERROR src/mp_segment.cpp -o $@ $(OPTIONS)

priv/mix_segment.so:
	mkdir -p priv && \
	$(CC) $(CFLAGS) $(ERLANG_FLAGS) $(CPPJIEBA_FLAGS) -shared -DLOGGER_LEVEL=LL_ERROR src/mix_segment.cpp -o $@ $(OPTIONS)
priv/hmm_segment.so:
	mkdir -p priv && \
	$(CC) $(CFLAGS) $(ERLANG_FLAGS) $(CPPJIEBA_FLAGS) -shared -DLOGGER_LEVEL=LL_ERROR src/hmm_segment.cpp -o $@ $(OPTIONS)

priv/query_segment.so:
	mkdir -p priv && \
	$(CC) $(CFLAGS) $(ERLANG_FLAGS) $(CPPJIEBA_FLAGS) -shared -DLOGGER_LEVEL=LL_ERROR src/query_segment.cpp -o $@ $(OPTIONS)
